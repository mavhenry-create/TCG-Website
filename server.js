  ...
  try {
    pool.query(
      ...
      (error, result) => {
        ...
        // Get current user's wishlist - fetch full card data
        pool.query(
          "SELECT id, name FROM wishlist WHERE user_id = $1",
          [req.session.userId],
          (err1, myWishlist) => {
            if (err1) {
            ...
            pool.query(
              ...
              (err2, friendWishlist) => {
                ...
                res.json({
                  ...
                  onlyTheyHave,
                  friendUsername,
                  myCards: decorate(myWishlist.rows, friendCardIds),
                  friendCards: decorate(friendWishlist.rows, myCardIds),},
                });
              },
            );
          },
